...
#
# A masked unit cannot be restarted. Custom units that live only in
# /etc/systemd/system/rembg.service are replaced by a /dev/null symlink
# when masked, so unmask can leave no unit file. Restore the copy shipped
# at UNIT_SRC only when systemd cannot load the unit after unmask.
set -euo pipefail

UNIT="${UNIT:-rembg.service}"
UNIT_SRC="${UNIT_SRC:-/opt/rembg/current/systemd/rembg.service}"
...
ensure_rembg_unit() {
  ...
  if unit_is_masked; then
    ...
    if ! sudo_systemctl unmask "$UNIT"; then
      echo "sudo systemctl unmask ${UNIT} failed. On the VM run: sudo systemctl unmask rembg.service"
      dump_unit_state
      return 1
    fi
  fi
